...
import io.bitchat.server.http.HttpContext;
import io.bitchat.server.http.converter.PrimitiveConverter;
import io.bitchat.server.http.converter.PrimitiveTypeUtil;
import io.bitchat.server.http.exception.InvocationException;
import io.bitchat.server.http.exception.ValidationException;
...
public class ProxyInvocation {
    ...
    private static class Invocation {
        ...
        private Object[] getParameters(Method method, Class<?>[] parameterTypes) throws Exception {
            ...
            //获取参数列表
            Map<String, List<String>> paramMap = HttpRequestUtil.getParameterMap(HttpContext.currentContext().getRequest());
            ...
            for (int i = 0; i < parameterTypes.length; i++) {
                ...
                if (annotation == null || annotation.length == 0) {
                    ...
                    if (PrimitiveTypeUtil.isPriType(type)) {
                        log.warn("Must specify a @Param annotation for primitive type parameter in method={}", method.getName());
                        continue;
                    }
                    // 封装对象类型的parameter
                    ...
                    try {
                        ...
                        if (param.notNull()) {
                            checkNull(param.key(), parameter);
                        }
                        if (param.notBlank()) {
                            checkBlank(param.key(), parameter);
                        }
                        parameters[i] = parameter;
                    } catch (Exception e) {
        ...
        private List<Class> getMethodGenericParameterTypes(Method method, int index) {
